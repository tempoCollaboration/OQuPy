...
def exact_2d_correlations(alpha, omega_cutoff, k, dt, shape):
    # Exact 2d correlations for an Ohmic spectral density and exponential cutoff at 0 temperature
    eta_func = lambda k: np.log(1/omega_cutoff+1.j*k*dt)
    if shape == "square":
        correl = eta_func(k-1)-2*eta_func(k)+eta_func(k+1)
    elif shape == "upper-triangle":
        correl = eta_func(k+1)-eta_func(k)-1.j*omega_cutoff*dt
