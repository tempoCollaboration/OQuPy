...
def test_gibbs_oqupy():
    ...
    gibbs_param = oqupy.GibbsParameters(temperature, 50, 1.0e-5, name="rough-A")

    gibbs_sys = oqupy.GibbsTempo(system=system,
                              bath=bath,
                              parameters=gibbs_param)
    assert gibbs_sys.dimension == 2
    gibbs_sys.compute(end_time=end_time1, progress_type="bar")
    dyn_A = oqupy_sys_A.get_dynamics()
