...
def _chain_rule(forwardprop_deriv_list,backprop_deriv_list,dprop_dparam_list):
    """
    Uses chain rule to stitch together forward and backprop
    """
    assert len(forwardprop_deriv_list) == len(dprop_dparam_list), \
                    'dprop_dpram_list must be the same length as the number of timesteps'
